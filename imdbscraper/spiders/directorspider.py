...
class DirectorSpider(scrapy.Spider):
    def generate_start_urls():
        ...
        filepath = (path / "../../data/json/directs.json").resolve()
        output = []
        ...
        with open(filepath) as json_file:
            ...
            for each in data:
                output.append(
                    f"https://www.imdb.com/name/{each['did']}/?ref_=nmls_hd")
    ...
    def parse(self, response):
        ...
        yield response.follow(
            url=f"{response.url.rsplit('?')[0]}bio?ref_=nm_ql_1", callback=self.parse_director_description, meta={"item": item})

    def parse_director_description(self, response):
        item = response.meta["item"]
        description = response.css(".soda.odd p::text").extract_first()
        item["description"] = description.strip(
        ) if description is not None else "Biography needs more update"
